...
2. Put ass_to_html.py file in a folder with .ass files and execute it (in the terminal or by double clicking it).
...
ass_files = [f for f in os.listdir('.') if f.endswith('.ass')]
if len(ass_files) != 0:
    htmlDir = os.path.join(os.getcwd(), 'ass_to_html')
    if not os.path.isdir(htmlDir):
        os.mkdir(htmlDir)
        
#Getting data from .ass files
for af in range(0, len(ass_files)):
    with open(ass_files[af], 'r', encoding='utf-8') as a:
        ass = a.read().splitlines()
    eventsLine = float('inf')
    data = []
    for (i, a) in enumerate(ass):
        if a == '[Events]':
            eventsLine = i + 2
            formatTags = (ass[i + 1])[8:].replace(' ', '').split(',')
        if eventsLine <= i:
            for (i, f) in enumerate(formatTags):
                if i == 0:
                    data.append({f: a[10:].split(',')[i]})
                else:
                    if (f == 'Start' or f == 'End') \
                        and len(a[10:].split(',')[i].split(':')[0]) < 2:
                        data[-1].update({f: '0' + a[10:].split(',')[i]})
                    else:
                        data[-1].update({f: a[10:].split(',')[i]})
                        
#Creating .html files with imported font, Noto Sans JP - https://fonts.google.com/specimen/Noto+Sans+JP
    with open("ass_to_html\\"+ass_files[af][:-4]+".html","w",encoding="utf-8") as html:
...
#Creating a .css file
if not os.path.isfile("ass_to_html\\subs.css"):
    with open("ass_to_html\\subs.css","w",encoding="utf-8") as css:
        css.write('''body,
html {
  width: 100%;
  height: 100%;
...
#Creating a .js file
if not os.path.isfile("ass_to_html\\script.js"):
    with open("ass_to_html\\script.js","w",encoding="utf-8") as js:
